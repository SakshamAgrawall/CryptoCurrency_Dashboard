            ...
            <form className="flex pl-2 w-full">
                <div className="flex flex-grow rounded-sm ">
                    <div className="flex flex-grow items-center">
                        ...
                        <input value={searchText} onChange={handleInput} required type="search" name='searchText' id='searchTextDesktop' className='flex border border-black shadow-lg focus:ring-1 md:text-md sm:text-md w-full pl-14 py-3 pr-2 p rounded-lg overflow-hidden focus-within:shadow-none outline-none sm:items-center' placeholder='Seacrh by coin ' style={{ marginLeft: "20px", marginTop: "20px" }} />
                    </div>
                </div>
            </form>

            {searchText.length ?(
                <ul id='searchbar' className='absolute top-14 -right-2 w-full h-96 mr-2 rounded overflow-x-hidden py-2 overflow-scroll scrollbar-thin backdrop-blur-md z-10'>
                    {searchData ? (
                        searchData.map((coin)=>{
                            return (
                                <li className='flex items-center ml-4 my-2 cursor-pointer' key={coin.id} onClick={()=>selectCoin(coin.id)}>
                                    <img src={coin.thumb} alt="coin" className='w-[1rem] h-[1rem] mx-1.5' />
                                    <span>{coin.id}</span>
                                </li>
                            );
                        })
